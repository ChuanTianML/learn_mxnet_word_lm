...
def rnn(bptt, vocab_size, num_embed, nhid,
    ...
    embed = mx.sym.Embedding(data=data, weight=weight, input_dim=vocab_size,    #构建word embedding，输出形状为（bptt, batch_size, embedding_size）未确定？
                             output_dim=num_embed, name='embed')

    # stacked rnn layers
    states = []                                                                                 #保存了每层输出的states，最终形状(num_layers*batch_size, 2, hidden_size)？
    state_names = []                                                                            #保存了每层输出的states的名字，最终形状 (num_layers*2, )
    outputs = mx.sym.Dropout(embed, p=dropout)                                                  #对embedding进行dropout，outputs变量名循环使用，形状为（bptt, batch_size, embedding_size）未确定？即一整层的输入
    for i in range(num_layers):
        ...
        outputs, next_states = cell.unroll( bptt,                                               #length=bptt 要展开的时间步数量
                                            ...
                                            begin_state=[begin_cell_state, begin_hidden_state], #(nested list of Symbol, default None) ，形状是(2, 1, batch_size, nhid)
        ...
        """
        outputs, next_states都是一整层的输出（即合并了时间步）
        outputs形状是（bptt, batch_size, hidden_size）
        next_states形状是(1, batch_size, hidden_size)
        """
        outputs = mx.sym.Dropout(outputs, p=dropout)                                            #对输出进行dropout
        states += next_states

    # decoder
    pred = mx.sym.Reshape(outputs, shape=(-1, nhid))                                            #形状转换：(bptt, batch_size, hidden_size) -> (bptt*batch_size, hidden_size)
    ...
    pred = mx.sym.Reshape(pred, shape=(-1, vocab_size))                                         #形状转换：(bptt*batch_size, vocab_size) -> (bptt*batch_size, vocab_size) 感觉不需要转换形状啊
...
def softmax_ce_loss(pred):
    ...
    label = mx.sym.Reshape(label, shape=(-1,))                  #把输入的label拉平成一维，(bptt, batch_size) -> (bptt*batch_size,)
    logits = mx.sym.log_softmax(pred, axis=-1)                  #计算logits（先softmax后取log），得到形状(bptt*batch_size, vocab_size)
    loss = -mx.sym.pick(logits, label, axis=-1, keepdims=True)  #将每个单词的-logits取出来，得到形状(bptt*batch_size, 1)
    loss = mx.sym.mean(loss, axis=0, exclude=True)              #遍历0维度，计算其余维度均值，得到形状(bptt*batch_size,)
